...
extract_ts2 <- function(f, xy, ts_type = "Water Surface", time_stamp = NULL) {
  ...
  do_extract <- function(.f) {
    ...
    time_series <- .f[[hdf_paths$RES_2D_FLOW_AREAS]][[model_flow_area_name]][[ts_type]][nearest_cell_index, time_idx]
    stacked_time_series <- matrix(t(time_series), ncol=1, byrow = TRUE)
    
    hdf_cell_index <- nearest_cell_index - 1
    
    tibble::tibble(
      "datetime" = rep(model_timestamps[time_idx], length(nearest_cell_index)), 
      "plan_id" = model_attributes$plan_id,
      "time_series_type" = ts_type, 
      "hdf_cell_index" = rep(hdf_cell_index, each = length(nearest_cell_index)), 
